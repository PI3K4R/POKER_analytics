...
stages = [pt.Preflop, pt.Flop, pt.Turn, pt.River]
def creating_variables_for_simulation(n, num_of_players: int, card_list: list, sql_con: sql.Cursor) -> list:
    ...
    while i < num_of_players:
        player = [pt.Card(card_list[i*2][0].upper(), dictionary[card_list[i*2][1]]), pt.Card(card_list[i*2+1][0].upper(), dictionary[card_list[i*2+1][1]])]
        player = sorted(player, key=lambda c: c.value)
        player_db = pt.subset_of_db(n, sql_con, None, player[0].rank, player[1].rank)
        i += 1
...
if __name__ == '__main__':
    ...
    if len(card_list)%2 != 0:
        ...
        results = pt.simulating_games_from_given_stage(stage, *creating_variables_for_simulation("7", num_of_players, card_list, cur))
